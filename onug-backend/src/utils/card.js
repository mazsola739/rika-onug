...
const anyMasonsInPlay = cardIds => includesAny(cardIds, [5, 6]) || ( cardIds.includes(1) && includesAny(cardIds, [5, 6]) )
const werewolvesInPlay = cardIds => includesAny(cardIds, [15, 16])
const multipleWerewolvesAtPlay = cardIds => includesAll(cardIds, [15, 16])
const getCardPositionNamesForPlayers = (gameState, playerToExclude) => Object.values(gameState.players).flatMap((player) => player.player_number === playerToExclude ? [] : player.player_number)
const getWerewolfCardPositionNamesForPlayers = (gameState) => Object.values(gameState.players).flatMap((player) => werewolvesInPlay([player.card.known_card.id]) ? player.player_number :  [] )
const getMasonCardPositionNamesForPlayers = (gameState) => Object.values(gameState.players).flatMap((player) => anyMasonsInPlay([player.card.known_card.id]) ? player.player_number :  [] )
