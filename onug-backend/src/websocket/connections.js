...
export const initWebSocketConnections = () =>
  ROOM_NAMES.forEach(
    (roomName) => (webSocketServerConnectionsPerRoom[roomName] = {})
  )
...
export const broadcast = (room_id, jsonMessage) => {
  ...
  logDebug(
    `broadcast to all users in room [${room_id}] with message [${JSON.stringify(
      jsonMessage
    )}]`
  )
  logTrace(
    `active connections in room [${room_id}]: ${JSON.stringify(
      Object.keys(webSocketServerConnectionsPerRoom?.[room_id])
    )}`
  )
...
export const sendMessageToPlayer = (room_id, token, jsonMessage) => {
  ...
  if (playerConnection) {
    logDebug(
      `Sending message to user [${room_id}][${token}] with message [${JSON.stringify(
        jsonMessage
      )}]`
    )
    ...
    logError(
      `No active WebSocket connection found for user [${room_id}][${token}]`
    )
  }
}
