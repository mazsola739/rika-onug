...
export const updateRoom = async (message) => {
  ...
  let totalPlayers = determineTotalPlayers(
    newGamestate.selected_cards.length,
    newGamestate.selected_cards
  )
  // TODO validate if player is admin
  if (expansion) {
    newGamestate.selected_expansions = toggleExpansions(
      newGamestate.selected_expansions,
      expansion
    )
    newGamestate.selected_cards = filterCardsByExpansions(
      newGamestate.selected_cards,
      newGamestate.selected_expansions
    )
  }
  ...
  totalPlayers = determineTotalPlayers(
    newGamestate.selected_cards.length,
    newGamestate.selected_cards
  )
