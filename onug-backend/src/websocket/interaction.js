...
const generateInteractionResponse = (gameState, token, selected_positions, ws) => {
  ...
  if (interaction_type === "ANNOYING_LAD")                return roles.thing_response(newGameState, token, selected_positions, 55)
  ...
  if (interaction_type === "DOPPELGÄNGER_REVEALER")       return roles.revealer_response(newGameState, token, selected_positions, 1)
  ...
  if (interaction_type === "REVEALER")                    return roles.revealer_response(newGameState, token, selected_positions, 24)
  if (interaction_type === "ROBBER")                      return roles.robber_response(newGameState, token, selected_positions, 8)
  if (interaction_type === "ROLE_RETRIEVER")              return roles.robber_response(newGameState, token, selected_positions, 66)
  ...
  if (interaction_type === "SWITCHEROO")                  return roles.troublemaker_response(newGameState, token, selected_positions, 68)
  ...
  if (interaction_type === "TROUBLEMAKER")                return roles.troublemaker_response(newGameState, token, selected_positions, 11)
