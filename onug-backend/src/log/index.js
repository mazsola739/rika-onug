...
const isLogLevelEnabled = (logLevel) => {
  switch (logLevel) {
    ...
    case LOG_LEVELS.DEBUG:
      return LOG_LEVEL === LOG_LEVELS.DEBUG || LOG_LEVEL === LOG_LEVELS.TRACE
    case LOG_LEVELS.TRACE:
      return LOG_LEVEL === LOG_LEVELS.TRACE
    default: {
      console.warn(
        `log level is not set to a valid value. Please check your ONUG_LOG_LEVEL env variable. tried to log a: [${logLevel}], LOG_LEVEL is set to: [${LOG_LEVEL}]`
      )
      return false
    }
  }
...
export const logError = (message, items = '') =>
  writeToLogFile(
    `[ERROR] ${message} ${items && JSON.stringify(items, null, 2)}`
  ) &&
  console.error(
    `${logTimestamp()}: [ERROR] ${message} ${JSON.stringify(items, null, 2)}`
  )
export const logWarn = (message, items = '') =>
  writeToLogFile(
    `[WARN]  ${message} ${items && JSON.stringify(items, null, 2)}`
  ) &&
  isLogLevelEnabled(LOG_LEVELS.WARN) &&
  console.warn(
    `${logTimestamp()}: [WARN] ${message} ${
      items && JSON.stringify(items, null, 2)
    }`
  )
export const logInfo = (message, items = '') =>
  writeToLogFile(
    `[INFO]  ${message} ${items && JSON.stringify(items, null, 2)}`
  ) &&
  isLogLevelEnabled(LOG_LEVELS.INFO) &&
  console.info(
    `${logTimestamp()}: [INFO] ${message} ${
      items && JSON.stringify(items, null, 2)
    }`
  )
export const logDebug = (message, items = '') =>
  writeToLogFile(
    `[DEBUG] ${message} ${items && JSON.stringify(items, null, 2)}`
  ) &&
  isLogLevelEnabled(LOG_LEVELS.DEBUG) &&
  console.info(
    `${logTimestamp()}: [DEBUG] ${message} ${
      items && JSON.stringify(items, null, 2)
    }`
  )
export const logTrace = (message, items = '') =>
  writeToLogFile(
    `[TRACE] ${message} ${items && JSON.stringify(items, null, 2)}`
  ) &&
  isLogLevelEnabled(LOG_LEVELS.TRACE) &&
  console.info(
    `${logTimestamp()}: [TRACE] ${message} ${
      items && JSON.stringify(items, null, 2)
    }`
  )
