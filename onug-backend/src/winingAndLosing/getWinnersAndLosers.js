...
export const getWinnersAndLosers = (gamestate) => {
  ...
  const winnerTeams = Array.from(
    new Set(
      voteResult
        .filter((player) => player.win && activeCards[player.player_number])
        .map((player) => activeCards[player.player_number].team)
    )
  )
