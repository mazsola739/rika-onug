...
export const empath = (gamestate, title, prefix) => {
  ...
  const randomPlayers = pickRandomUpToThreePlayers(
    totalPlayers,
    'conjunction_and'
  )
  const empathKey = randomKey === 'activePlayers' ? randomPlayers : [randomKey]
  const randomEmpathInstruction = getRandomItemFromArray(
    randomEmpathInstructions
  )
  ...
  if (randomKey === 'activePlayers') {
    activePlayerNumbers = [
      randomPlayers.map((player) =>
        parseInt(player.replace('identifier_player', '').replace('_text', ''))
      ),
    ]
  } else if (
    randomKey === 'identifier_oddplayers_text' ||
    randomKey === 'identifier_evenplayers_text' ||
    randomKey === 'identifier.everyone_text'
  ) {
    const evenOdd = randomKey.includes('even')
      ? 'even'
      : randomKey.includes('odd')
        ? 'odd'
        : ''
  ...
  tokens.forEach((token) => {
    ...
    if (activePlayerNumbers.includes(playerNumber)) {
      ...
      const isNotDoppelgangerEmpath =
        prefix === 'doppelganger_empath' &&
        card.player_role_id !== 20 &&
