...
export const seerResponse = (
  ...
  const playerCards = selected_card_positions.some((pos) =>
    pos.includes('player')
  )
  const centerCards = selected_card_positions.some((pos) =>
    pos.includes('center')
  )
  const playerHistory =
  ...
  if (
    ...
    selected_card_positions.every((position) =>
      playerHistory.includes(position)
    )
  ) {
