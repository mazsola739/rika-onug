...
export const getSelectablePlayersWithNoArtifact = (players, artifactedCards) =>
  players.filter((player) => !artifactedCards.includes(player))
...
export const getRandomArtifact = (playerArtifacts) => {
  ...
  const availableArtifacts = artifacts.filter(
    (artifact) => !assignedArtifacts.includes(artifact.id)
  )
