...
import {
  bodysnatcherKeys,
  randomBodysnatcherInstructions,
} from './bodysnatcher.constants'
...
export const bodysnatcher = (gamestate, title, prefix) => {
  ...
  const randomBodysnatcherInstruction = getRandomItemFromArray(
    randomBodysnatcherInstructions
  )
  ...
  const narration = [
    ...
    randomBodysnatcherInstruction === 'bodysnatcher_steal_text'
      ? bodysnatcherKey
      : '',
  ...
  tokens.forEach((token) => {
    ...
    if (prefix === 'bodysnatcher') {
      if (
        card.player_original_id === 74 ||
        (card.player_role_id === 74 &&
          COPY_PLAYER.includes(card.player_original_id))
      ) {
        newGamestate.players[token].action_finished = false
        interaction = bodysnatcherInteraction(
          newGamestate,
          token,
          title,
          randomBodysnatcherInstruction,
