...
//TODO neighbors
export const morticianResponse = (
  gamestate,
  token,
  ...
  const cardPositions = selected_card_positions.slice(
    0,
    gamestate.players[token].player_history[title].selectable_card_limit.player
  )
  ...
  const shouldResetPlayerCardId = () => {
    if (
      viewCards.some(
        (card) =>
          newGamestate.players[token].card.player_original_id === card.id
      )
    ) {
      return true
    }
    if (
      ...
      currentPlayerNumber === cardPositions[0] &&
      viewCards[0].card.id ===
