...
export const rascalResponse = (
  ...
  switch (newGamestate.players[token].player_history[title].random) {
    case 'troublemaker': {
      ...
      if (
        currentPlayerNumber === position1 ||
        currentPlayerNumber === position2
      ) {
    ...
    case 'witch':
      if (!newGamestate.players[token].player_history[title].witch_answer) {
        ...
        newGamestate.players[token].player_history[title].witch_answer
      ) {
        const firstSelectedPositionCard =
          newGamestate.card_positions[
            newGamestate.players[token].player_history[title].selected_card
          ].card
        const secondSelectedPositionCard =
          newGamestate.card_positions[selected_card_positions[0]].card
        ...
        if (selected_card_positions[0] === currentPlayerNumber[0]) {
          const currentCard =
            newGamestate.card_positions[currentPlayerNumber[0]].card
    ...
    case 'robber': {
      ...
      const selectedCard = {
        ...newGamestate.card_positions[selectedPosition].card,
      }
      ...
      if (
        newGamestate.players[token].player_history[title].random === 'drunk'
      ) {
        newGamestate.players[token].card.player_card_id = 87
      } else {
