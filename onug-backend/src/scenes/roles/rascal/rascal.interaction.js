...
export const rascalInteraction = (gamestate, token, title) => {
  ...
  const getSelectableTwoPlayers = (rascalKey) => {
    switch (rascalKey) {
      ...
      case 'identifier_any2odd_text': {
        const evenOrOddTwo = rascalKey
          .replace('identifier_any2', '')
          .replace('_text', '')
      ...
      case 'identifier_any2lower_text': {
        const higherOrLowerTwo = rascalKey
          .replace('identifier_any2', '')
          .replace('_text', '')
        return getAnyHigherOrLowerPlayerNumbersByToken(
      ...
      case 'identifier_bothneighbors_text': {
        const directionTwo = rascalKey.includes('left')
          ? 'left'
          : rascalKey.includes('right')
            ? 'right'
            : 'both'
        const amountTwo = rascalKey.includes('2') ? 2 : 1
        return getPlayerNeighborsByToken(
          newGamestate.players,
          directionTwo,
          amountTwo
        )
  ...
  const getSelectableOnePlayers = (rascalKey) => {
    switch (rascalKey) {
      ...
      case 'identifier_lower_text': {
        const higherOrLowerOne = rascalKey
          .replace('identifier_', '')
          .replace('_text', '')
        return getAnyHigherOrLowerPlayerNumbersByToken(
          newGamestate.players,
      ...
      case 'identifier_anyodd_text': {
        const evenOrOddOne = rascalKey
          .replace('identifier_any', '')
          .replace('_text', '')
      ...
      case 'identifier_rightneighbor_text': {
        const directionOne = rascalKey.includes('left')
          ? 'left'
          : rascalKey.includes('right')
            ? 'right'
            : 'both'
  ...
  if (randomRascalInstruction === 'rascal_troublemaker_text') {
    selectableCards =
      getSelectablePlayersWithNoShield(selectableTwoPlayers) <= 2
        ? []
        : getSelectablePlayersWithNoShield(selectableTwoPlayers)
    limit = selectableCards.length >= 2 ? 2 : 0
    privateMessage = [
      selectableCards.length >= 2
        ? 'interaction_may_two_any'
        : 'interaction_no_selectable_player',
    ]
  } else if (
    randomRascalInstruction === 'rascal_drunk_text' ||
    randomRascalInstruction === 'rascal_robber_text'
  ) {
    if (newGamestate.players[token].shield) {
      ...
      selectableCards =
        rascalKey === 'identifier_center_text'
          ? CENTER_CARD_POSITIONS
          : getSelectableOtherPlayerNumbersWithNoShield(
              selectableOnePlayers,
              token
            )
      privateMessage = [
    ...
    selectableCards =
      rascalKey === 'identifier_center_text'
        ? CENTER_CARD_POSITIONS
        : getSelectablePlayersWithNoShield(selectableOnePlayers)
    privateMessage = [
      selectableCards.length === 0
        ? 'interaction_no_selectable_player'
        : 'interaction_may_one_any_other',
  ...
  const random = randomRascalInstruction
    .replace('rascal_', '')
    .replace('_text', '')
