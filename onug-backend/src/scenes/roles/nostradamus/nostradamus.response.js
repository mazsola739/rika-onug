...
export const nostradamusResponse = (
  gamestate,
  token,
  ...
  if (GOOD_GUY.includes(playerOneCardId)) {
    if (!GOOD_GUY.includes(playerTwoCardId)) {
      ...
      newGamestate.players[token].card.player_role =
        newGamestate.card_positions[selected_card_positions[1]].card.role
      newGamestate.players[token].card.player_team =
        newGamestate.card_positions[selected_card_positions[1]].card.team
    } else if (GOOD_GUY.includes(playerTwoCardId)) {
      if (!GOOD_GUY.includes(playerThreeCardId)) {
        showCards = selectedCards
        newGamestate.players[token].card.player_role =
          newGamestate.card_positions[selected_card_positions[2]].card.role
        newGamestate.players[token].card.player_team =
          newGamestate.card_positions[selected_card_positions[2]].card.team
      } else {
        showCards = selectedCards
        newGamestate.players[token].card.player_team =
          newGamestate.card_positions[selected_card_positions[2]].card.team
    ...
    newGamestate.players[token].card.player_role =
      newGamestate.card_positions[selected_card_positions[0]].card.role
    newGamestate.players[token].card.player_team =
      newGamestate.card_positions[selected_card_positions[0]].card.team
  }
  ...
  newGamestate.players[token].player_history[title] = {
    ...
    viewed_cards:
      showCards.length > 1
        ? selected_card_positions.slice(0, 2)
        : [selected_card_positions[0]],
  }
  ...
  const interaction = generateRoleInteraction(newGamestate, token, {
    private_message: [
      ...
      showCards.length >= 2
        ? formatPlayerIdentifier(selected_card_positions)[1]
        : '',
      showCards.length === 3
        ? formatPlayerIdentifier(selected_card_positions)[2]
        : '',
    ],
    showCards,
  })
