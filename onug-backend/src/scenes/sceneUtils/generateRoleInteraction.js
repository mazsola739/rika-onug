...
const isPlayersCardsFlipped = (flipped, playersPosition) =>
  Object.keys(flipped).some((key) => playersPosition === key)

const isActivePlayersCardsFlipped = (flipped, playersPosition) =>
  flipped.some((obj) => Object.keys(obj)[0] === playersPosition)
...
const updatePlayerCard = (gamestate, token) => {
  ...
  const iSeeMyCardIsFlipped = isActivePlayersCardsFlipped(
    flippedCards,
    currentPlayerNumber
  )
  const iSeeMyCardElsewhere = isPlayersCardsFlipped(
    flippedCards,
    currentPlayerNumber
  )
...
export const generateRoleInteraction = (
  ...
  const informations = {
    ...
    show_cards:
      showCards !== null
        ? concatArraysWithUniqueElements(showCards, gamestate.flipped)
        : gamestate.flipped,
