export const getPlayerNeighborsByToken = (
  players,
  token,
  direction,
  amount
  ...
  if (direction === 'left' || direction === 'both') {
    ...
    while (
      prevNeighborNumber !== playerNumber - amount &&
      prevNeighborNumber !== playerNumber
    ) {
      if (prevNeighborNumber < 1) {
        prevNeighborNumber = playerCount
      }
      const prevNeighborId = Object.keys(players).find(
        (key) => players[key].player_number === prevNeighborNumber
      )
      if (prevNeighborId) {
  ...
  if (direction === 'right' || direction === 'both') {
    ...
    while (
      nextNeighborNumber !== playerNumber + amount &&
      nextNeighborNumber !== playerNumber
    ) {
      if (nextNeighborNumber > playerCount) {
        nextNeighborNumber = 1
      }
      const nextNeighborId = Object.keys(players).find(
        (key) => players[key].player_number === nextNeighborNumber
      )
      if (nextNeighborId) {
