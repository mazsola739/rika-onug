...
exports.witch = (gameState, tokens) => {
  ...
  tokens.forEach((token) => {
    ...
    const iSeeMyCardIsFlipped = isActivePlayersCardsFlipped(newGameState.flipped, witchPlayerNumber)
    const iSeeMyCardElsewhere = isPlayersCardsFlipped(newGameState.flipped, witchPlayerNumber)
  
    if (iSeeMyCardIsFlipped) {
      newGameState.players[token].card.id = newGameState.card_positions[witchPlayerNumber[0]].id
      newGameState.players[token].card.role_id = newGameState.card_positions[witchPlayerNumber[0]].id
      newGameState.players[token].card.role = newGameState.card_positions[witchPlayerNumber[0]].role
      newGameState.players[token].card.team = newGameState.card_positions[witchPlayerNumber[0]].team
