...
const { getPlayerNumbersWithNonMatchingTokens, getPlayerTokenByPlayerNumber, getSelectablePlayersWithNoShield } = require("../utils")
...
exports.sentinel_response = (gameState, token, selected_positions) => {
  if (selected_positions.every((position) => gameState.players[token].role_history.selectable_cards.includes(position)) === false) {
    logError("Invalid selected positions in sentinel_response function")
    return gameState
  }
