...
export const flipper = (gameState, prefix) => {
  ...
  tokens.forEach(token => {
   ...
   if (newGameState.players[token].card.player_original_id === 59) {
    newGameState.players[token].scene_role_interaction.interaction = flipper_interaction(newGameState, token)
   }
  })
...
export const flipper_interaction = (gameState, token) => {
  const newGameState = { ...gameState }
...
export const flipper_response =  (gameState, token, selected_positions) => {
