...
export const selfawarenessgirl = (gameState, title, hasDoppelganger) => {
  ...
  tokens.forEach((token) => {
    ...
    if (newGameState.players[token].card.player_original_id === 67 || (newGameState.players[token].card.role_id === 67 && newGameState.players[token].card.player_original_id === 30) || (newGameState.players[token].card.role_id === 67 && newGameState.players[token].card.player_original_id === 64)) {
