export const ripple_1minute_text =
  'You will only have one minute before you have to vote.'
export const ripple_app_text =
  'When it is time to vote, you may vote for the app instead of a player. If the app receives the most votes, no one is killed.'
export const ripple_doublevote_text =
  'When it is time to vote, use two hands to point, with each hand counting as one vote.'
export const ripple_drunk_text = 'You must exchange your card with a card from'
export const ripple_drunk_end_text = 'but you may not look at either card.'
export const ripple_dualshuffle1_text =
  'show your cards to each other, then give both to'
export const ripple_dualshuffle2_text =
  'shuffle both cards face down. Give one to'
export const ripple_dualshuffle3_text =
  'and place the other in front of yourself.'
export const ripple_dualseer_text = 'together, you may view one card from'
export const ripple_faceaway_text =
  'You must face away from the table until after the vote.'
export const ripple_iamalien_text =
  'From now until the vote, you may only say this phrase: "I am an alien."'
export const ripple_insomniac_text = 'You may secretly view your own card.'
export const ripple_intro_text =
  'There has been a ripple in the space-time continuum!'
export const ripple_nospeak_text = 'You may not speak until after the vote.'
export const ripple_openeyes_text =
  'Keep your eyes open from this point forward.'
export const ripple_repeat_text =
  "You've entered a time loop and must repeat the entire night."
export const ripple_repeatrole_text =
  'you get to repeat your night action. Whatever you did when you woke up before, do exactly same again, right now'
export const ripple_repeat_2_text =
  'Some roles may not wake up during the time loop.'
export const ripple_revealer_text = 'You may turn over one card from'
export const ripple_revealer_end_text =
  'If that card is on the Villager team, leave it face up. Otherwise, turn it back over, face down.'
export const ripple_robber_text = 'You may steal a card from'
export const ripple_robber_end_text =
  'If you do, look at your new card. Do not wake up if that role is called.'
export const ripple_troublemaker_text = 'You may exchange cards between'
export const ripple_troublemaker_end_text = 'Do not look at either card.'
export const ripple_view1_text = 'You may view one card from'
export const ripple_view2_text = 'You may view two cards from'
export const ripple_witch_text =
  'You may view one center card. If you do, you must give it to'
